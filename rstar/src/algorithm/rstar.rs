    ...
    T: RTreeObject,
{
    ...
    let zero: <<T::Envelope as Envelope>::Point as Point>::ComposedScalar = Zero::zero();
    let insertion_envelope = to_insert.envelope();
    let mut inclusion_count = 0;
    let mut min_area = <<T::Envelope as Envelope>::Point as Point>::ComposedScalar::max_value();
    let mut min_index = 0;
    ...
    Params: RTreeParams,
{
    ...
    let zero = <<T::Envelope as Envelope>::Point as Point>::ComposedScalar::zero();
...
where
    ...
    Params: RTreeParams,
{
    let mut best_goodness =
        <<T::Envelope as Envelope>::Point as Point>::ComposedScalar::max_value();
    let mut best_axis = 0;
    let min_size = Params::MIN_SIZE;
